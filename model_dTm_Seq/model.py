...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, node_dim, n_head, pair_dim, num_layer):
        ...
        self.fixed_embedding_transform = torch.nn.Sequential(
            torch.nn.Linear(node_dim + 7 + 32, node_dim),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(node_dim, node_dim)
        )
        self.finetune_p = torch.nn.Sequential(
            torch.nn.Linear(node_dim + 1, node_dim),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(node_dim, node_dim)
        )
        self.pair_encoder = torch.nn.Linear(7, pair_dim)
        self.blocks = torch.nn.ModuleList([PretrainGeometricAttention(node_dim, n_head, pair_dim) for _ in range(num_layer)])
        self.dG_readout = torch.nn.Sequential(
            torch.nn.Linear(node_dim, node_dim),
            torch.nn.LeakyReLU(),
